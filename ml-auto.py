...
st.set_page_config(page_title='Prediciting Viability of Targets/MOA',
    layout='wide')

#---------------------------------#
st.write("""
# Prediciting Viability of Targets/MOA
**(Regression Edition)**
In this implementation, the *RandomForestClassifier()* function is used in this app for build a classification model using the **Random Forest** algorithm.
""")
...
#parameter_criterion = st.sidebar.select_slider('Performance measure (criterion)', options=['accuracy_score'])
parameter_bootstrap = st.sidebar.select_slider('Bootstrap samples when building trees (bootstrap)', options=[True, False])
parameter_oob_score = st.sidebar.select_slider('Whether to use out-of-bag samples for oob_score', options=[False, True])
...
def build_model(df):
    ...
    st.write('F1Score:')
    st.info( f1_score(Y_test, Y_pred_test) )
    ...
    #st.info( recall_score(Y_test, Y_pred_test) )
    ...
    grid_reset.columns = ['max_features', 'n_estimators', 'accuracy_score']
